...
async def upload_kpi_evaluation(
    ...
    if department.lower() == "contabilidad" and file_ext in [".xlsx", ".xlsm"]:
        try:
            ...
            for row in ws.iter_rows(values_only=True):
                if not row or len(row) < 9:
                    continue
                # If row[1] is a number, it's a KPI row
                if str(row[1]).isdigit():
                    kpi_name = str(row[2]) if row[2] else ""
                    freq = str(row[3]) if row[3] else ""
                    target = str(row[4]) if row[4] else ""
                    ...
                    comp_month = safe_float(row[6])
                    comp_weight = safe_float(row[7])
                    status_kpi = str(row[8]) if row[8] else ""
            ...
            print(f"Error procesando Excel de KPIs: {e}")
